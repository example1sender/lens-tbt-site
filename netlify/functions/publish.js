...
exports.handler = async function (event) {
  ...
  try {
    const store = getStore("tlp-settings", { siteID: SITE_ID, token: BLOB_TOKEN });
    await store.setJSON("current", payload);
  } catch (e) {
    return json(500, { error: "store_failed", v: "creds2", hasSite: !!SITE_ID, hasToken: !!BLOB_TOKEN, message: String(e && e.message || e) });
  }
  return json(200, { ok: true, v: "creds2", updated: payload.updated });
};
